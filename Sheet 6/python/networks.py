...
class SequentialNet:
  ...
  def add_conv2D(self, fshape, afun=None, optim=None, initializer=None, eval_method='scipy'):
    ...
    self.layers.append(Conv2DLayer(tensor, fshape, afun, optim, initializer, eval_method))
    self.no = (m, zh, zw)
  ...
  def backprop(self, x, y):
    ...
    delta = (self.evaluate(x) - y)/y.shape[0] 
  ...
  def train(self, x, y, batch_size=16, epochs=10):
    ...
    for e in range(epochs):
      ...
      print('Epoch {e}/{eps}'.format(e=e+1, eps=epochs))
      p = np.random.permutation(n_data)
      for j in range(n_batches):
        
        print('Batch {b}/{bt}'.format(b=j+1, bt=n_batches), end='\r')
