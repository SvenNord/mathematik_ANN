...
class DropoutLayer:
    ...
    def evaluate(self, a):
      ...
      if self.trainable:
        ...
        self.mask = np.random.binomial(1, self.p, size=shape)
        drop_a = a * self.mask
        return drop_a
      else:
        return p * a
...
class Conv2DLayer:
    ...
    def __init__(self, tensor, fshape, afun=None, optim=None, initializer=None):
    ...
    def backprop(self, delta):
        ...
        # Compute df = delta * f^~. df has shape (m, c, fh, fw)
        for k in range(c):
            for j in range(m):
                for i in range(n):
...
if __name__ == "__main__":
    ...
    test_layer = Conv2DLayer((c,h,w),(m, fh, fw), afun=Abs())
    test_layer.f = f
    test_layer.b = b

    # forward step using the method evaluate of the layer
    a=Conv2DLayer.evaluate(test_layer,x)
